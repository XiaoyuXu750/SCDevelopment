...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  saveRDS(SC_Cog_results.df, paste0(interfileFolder, "/SC_Cog_results_", Cogvar,"_CV", CVthr,"_SA", ds.resolution,"_cog2SC.rds"))
  length(SC_Cog_results.df$cognition_var[SC_Cog_results.df$anova.cov.p.fdr<0.05])}else{
    SC_Cog_results.df <- readRDS(paste0(interfileFolder, "/SC_Cog_results_", Cogvar,"_CV", CVthr,"_SA", ds.resolution,"_cog2SC.rds"))
  }
...
# 1             7 gam.smooth.t -0.4045436 0.03273799

# ds.resolution Interest.var r.spearman   p.spearman
# 1            17 gam.smooth.t -0.2250144 0.005167483
### plot: scatter plot+matrix graph
########################
correlation.df <- SCrankcorr(SC_Cog_results.df, "gam.smooth.t", ds.resolution,dsdata=TRUE)
...
ggsave(paste0(FigureFolder, '/cognition/', Cogvar, '_rev/CorrTvalue_SCrankcorr_n', ds.resolution, '_siteall.svg'), Fig, dpi=600, width=17.5, height =15, units = "cm")
