...
if (str_detect(wdpath, "ibmgpfs")){
  ...
  FigureFolder<-paste0('/ibmgpfs/cuizaixu_lab/xuxiaoyu/SC_development/Figure_ABCD_final/SA12/CV', CVthr)
...
SCdata <- SCdata %>% left_join(select(Behavior, subID, nihtbx_fluidcomp_uncorrected), by="subID")
...
for (i in 1:10){
  ...
  Fig <- ggplot(data=plotdf.tmp)+
    ...
    scale_y_continuous(breaks = c(0.9, 1.0, 1.1), limits=c(0.89, 1.15))+
    labs(x=NULL, y="SC strength (ratio)", color="Cognition")+
    mytheme
  
  print(Fig)
  ggsave(paste0(FigureFolder, '/cognition/', Cogvar, '_base/Interaction/developmentcurve_decile', i, '.tiff'),Fig, width = 10, height = 10, units = "cm")
