...
---
This script is to calculate associations between fluid cognition and SC strength in the ABCD dataset. GAM models were used to regress covariates. Statistical indexes will be generated.
...
cogagemodel <- gam(nihtbx_fluidcomp_uncorrected~s(age,k=3, fx=TRUE)+gender+handness+race+mean_fd, data=SCdata.cog)
...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  saveRDS(SC_Cog_results.df, paste0(interfileFolder, "/SC_Cog_results_", Cogvar,"_CV", CVthr,".rds"))
...
SCrankresult.whole<-SCrankcorr(SC_Cog_results.df.whole,"gam.smooth.t", 12, perm.id.full)
print(paste("Correlation coefficient between cognitve associations and connectional axis is", round(SCrankresult.whole$r.spearman,2), "with a P value of", round(SCrankresult.whole$p.spin,3)))
```

## 2. Plot: scatter plot+matrix graph, Figure 6 (b~c)

```{r plot}
correlation.df <- SCrankcorr(SC_Cog_results.df, "gam.smooth.t", 12, perm.id.full,dsdata=TRUE)
...
Fig
ggsave(paste0(FigureFolder, '/cognition/', Cogvar, '_rev/CorrTvalue_SCrankcorr_n12_siteall.tiff'), Fig, width=17, height =14, units = "cm")
ggsave(paste0(FigureFolder, '/cognition/', Cogvar, '_rev/CorrTvalue_SCrankcorr_n12_siteall.svg'), Fig, dpi=600, width=width, height =height, units = "cm")
...
Fig
filename<-paste0(FigureFolder, '/cognition/', Cogvar, '_rev/CorrTvalue_Matrix_n12_siteall.tiff')
