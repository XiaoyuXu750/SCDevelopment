...
---
This script is to calculate associations between fluid cognition and SC strength in the ABCD dataset. GAM models were used to regress covariates. Statistical indices will be generated. Fig. 4(B, C), Fig. S7(E), Fig. S10(E)
...
```{r cog_corr}
EFvars <- c("nihtbx_list_uncorrected", "nihtbx_cardsort_uncorrected", "nihtbx_flanker_uncorrected")
SCdata.EF <- SCdata[,c("scanID", EFvars)]
corr.test(SCdata.EF[,2:4])
EFPCA <- principal(SCdata.EF[,2:4], nfactors = 3, rotate = "none")
print(EFPCA) # PC1 can explain 56% variance
SCdata$EFPCA <- EFPCA$scores[,1]
SCdata$EF3add <- rowSums(SCdata[,EFvars])
...
## 2. Plot: scatter plot+matrix graph, Fig. 4(B, C), Fig. S7(E), Fig. S10(E)
