...
#### load data
CVthr = 75
derivative.posterior.df <- readRDS(paste0(resultFolder, '/derivative.posterior.df.SA', ds.resolution, '_CV', CVthr,'_validat_', addcovariate, '.rds'))
derivative.df <- readRDS(paste0(resultFolder, '/derivative.df', elementnum, '_CV', CVthr,'_validat_', addcovariate, '.rds'))
#### source function
...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  write.csv(deri.SCrank.posterior.corr, paste0(resultFolder, '/deri.SCrank', ds.resolution, '_CV', CVthr,'.posterior.diw.corr_', addcovariate, '.csv'), row.names = F)
}

###### extract age of maximal / zero S-A connectional axis: posterior median value + 95% CI
deri.SCrank.posterior.corr <- read.csv(paste0(resultFolder, '/deri.SCrank', ds.resolution, '_CV', CVthr,'.posterior.diw.corr_', addcovariate, '.csv'))
...
ggsave(paste0(FigureFolder,'/CV', CVthr, '/Alignment_development/SA', ds.resolution, '_posDeriv_divweight_corr_', addcovariate, '.tiff'), width=12, height=12, units="cm")
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/SA', ds.resolution, '_posDeriv_divweight_corr_', addcovariate, '.svg'), width=13.5, height=15, units="cm")
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/deri.diw_corr_SCrank', ds.resolution, 'ageAll_', addcovariate, '.tiff'), width = 14, height = 12, units="cm")
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/deri.diw_corr_SCrank', ds.resolution, 'ageAll_', addcovariate, '.svg'), dpi=600, width = 14, height = 12, units="cm")
#############################################
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/derivative_diw_SA', ds.resolution, '_changerate_', addcovariate, '.tiff'), width = 20, height = 16, units="cm")
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/derivative_diw_SA', ds.resolution, '_changerate_', addcovariate, '.svg'), width = 13, height = 13, units="cm")
write.csv(derivative.df.merge, paste0(interfileFolder, '/derivative_df_', ds.resolution, '_merge_CV', CVthr,'_', addcovariate, '.csv'), row.names = F)
