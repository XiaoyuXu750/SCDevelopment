...
This script conducted age-resolved developmental analysis. Spearman correlation coefficients between derivatives / posterior derivatives will be calculated at 1,000 age points within age spans.
Figure 4 a~d, Supplementary Figure S3 d~f.
...
## 1. calculate correlation between posterior derivatives and SC rank
```{r compute_corr}
...
# 2. compute correlation coefficients between connectional axis and 1,000 posterior derivatives at 1,000 age points.
...
###### extract age of zero SC rank correlation: posterior median value + 95% CI
...
age.0.corr.diw.median # 15.6162 (CV75) ; 16.05931 (CV25)
age.0.corr.diw.CI # 15.35310 15.93468 (CV75) ; 15.76817 16.43318 (CV25)
...
##### plot alignment with SC rank correlation (Figure 4 (b))
#############################################
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/SA12_posDeriv_divweight_corr.svg'), dpi=600, width=15, height =15, units="cm")
...
## 2. correlation plot at different age (Figure 4 (c,d))
```{r age-specific_plot}
...
SCrankcorr(df.age8, "derivative", 12, perm.id.full) #r=-0.7164879  0.011625
# CV25 r=-0.7462441 0.00025
df.poscorr.diw$age[which.min(abs(df.poscorr.diw$median-0))]
# age = 15.6162
ntmp<-which.min(abs(agerange-age.0.corr.diw.median)) # 545
agerange[ntmp]
...
df.age15 <- df.age15[-1,]
SCrankcorr(df.age15, "derivative", 12, perm.id.full) #r=-0.01239314 0.4342
#CV25 r=-0.005159593 0.7248
# age = 21.92
...
df.age21 <- df.age21[-1,]
SCrankcorr(df.age21, "derivative", 12, perm.id.full) #r=0.78766 0.000525
# CV25 r=0.7117204      0
...
minderiv <- min(c(df.age8$derivative, df.age15$derivative, df.age21$derivative))
maxderiv <- max(c(df.age8$derivative, df.age15$derivative, df.age21$derivative))
valuesderiv <- colorbarvalues(seq(minderiv, maxderiv, length.out=100), 0.3175)
...
# age 21.9
Matsize <- 12
matrix_Deriv.age21 <- Matrix12
matrix_Deriv.age21[indexsave12] <- df.age21$derivative
matrix_Deriv.age21[indexup12] <- t(matrix_Deriv.age21)[indexup12]
colnames(matrix_Deriv.age21) <-seq(1, Matsize)
rownames(matrix_Deriv.age21) <-seq(1, Matsize)
matrix_Deriv.age21.df <- as.data.frame(matrix_Deriv.age21)
matrix_Deriv.age21.df$nodeid <- seq(1, Matsize)
matrix_Deriv.age21.df.melt <- melt(matrix_Deriv.age21.df,id.vars=c("nodeid"))
matrix_Deriv.age21.df.melt$variable<-as.numeric(matrix_Deriv.age21.df.melt$variable)
matrix_Deriv.age21.df.melt$nodeid<-0-matrix_Deriv.age21.df.melt$nodeid
ggplot(data =matrix_Deriv.age21.df.melt)+
  geom_tile(aes(x=variable, y=nodeid, fill = value, color=value))+
  scale_fill_distiller(type="seq", palette = "RdBu",limit=c(minderiv, maxderiv), values=valuesderiv)+
  scale_color_distiller(type="seq", palette = "RdBu",limit=c(minderiv, maxderiv), values=valuesderiv)+
  geom_segment(aes(x = 0.5 , y = -0.5 , xend = Matsize+0.5 ,yend = -0.5-Matsize), color="black", linewidth=0.5)+
  ...
  theme(axis.line = element_line(linewidth = 0), 
        ...
        panel.grid.major=element_line(linewidth = 0), 
        panel.grid.minor=element_line(linewidth = 1))
ggsave(paste0(FigureFolder,'/CV', CVthr,   '/Alignment_development/Deri_SA12_diw_age21.tiff'),  height = 13, width = 15, units = "cm")

```

## 3. derivative plots (Figure 4 (a))
```{r derivative_plot}
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/derivative_diw_SA12_changerate.svg'), dpi=600, width=14, height =14, units="cm")
