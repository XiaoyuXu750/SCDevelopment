...
Figure 3 A~D, Supplementary Figure S7 B.
...
age.0.corr.diw.median # 15.5192 (CV75) ; 15.99007 (CV25)
age.0.corr.diw.CI # 15.29771 15.79621 (CV75) ; 15.71313 16.33660 (CV25)
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/SA12_posDeriv_divweight_corr.svg'), dpi=600, width=mywidth, height =myheight, units="cm")
...
## 2. correlation plot at different age (Figure 3 (C, D))
```{r age-specific_plot}
agerange <- unique(derivative.df$age)
df.poscorr.diw$median[df.poscorr.diw$age==min(agerange)] #-0.6477
# age = 8.083333
min(agerange)
df.age8 <-as.data.frame(matrix(NA, nrow=1, ncol=ncol(derivative.df)))
...
agerange[ntmp]
df.age21 <-as.data.frame(matrix(NA, nrow=1, ncol=ncol(derivative.df)))
...
df.age21 <- df.age21[-1,]
SCrankcorr(df.age21, "derivative", 12)
...
ggplot(data=derivative.df.merge)+
  ...
  theme(axis.text=element_text(size=21.2, color="black"), 
        axis.title =element_text(size=21.2),aspect.ratio = 0.65,
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/derivative_diw_SA12_changerate.svg'), dpi=600, width=20, height =16, units="cm")
