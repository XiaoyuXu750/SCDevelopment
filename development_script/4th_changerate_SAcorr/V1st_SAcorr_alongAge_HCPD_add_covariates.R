## Validation for additional covariates in the HCP-D dataset.
library(tidyverse)
library(R.matlab)
library(psych)
library(gratia)
...
#### calculate  in ds.resolution*ds.resolution
########################
...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  write.csv(deri.SCrank.posterior.corr, paste0(resultFolder, '/deri.SCrank', ds.resolution, '_CV', CVthr,'.posterior.diw.corr_validat_', addcovariate, '.csv'), row.names = F)
}

###### extract age of maximal / zero S-A connectional axis correlation: posterior median value + 95% CI
deri.SCrank.posterior.corr <- read.csv(paste0(resultFolder, '/deri.SCrank', ds.resolution, '_CV', CVthr,'.posterior.diw.corr_validat_', addcovariate, '.csv'))
...
age.0.corr.diw.median # 15.53312 (ICV); 15.47773 (SES)
age.0.corr.diw.CI # 15.31156 15.78237 (ICV); 15.24233 15.75467 (SES)
...
ggsave(paste0(FigureFolder,'/CV', CVthr, '/Alignment_development/SA', ds.resolution, '_posDeriv_divweight_corr_validat_', addcovariate, '.tiff'), width=12, height=12, units="cm")
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/SA', ds.resolution, '_posDeriv_divweight_corr_validat_', addcovariate, '.svg'), dpi=600, width=17.5, height=15, units="cm")
...
ggsave(paste0(FigureFolder,'/CV', CVthr,  '/Alignment_development/Agedistribution_0corr_validat_', addcovariate, '.tiff'), width=6, height = 6, units="cm")
ggsave(paste0(FigureFolder, '/CV', CVthr, '/Alignment_development/Agedistribution_0corr_validat_', addcovariate, '.svg'), width=6, height =6, units="cm")
