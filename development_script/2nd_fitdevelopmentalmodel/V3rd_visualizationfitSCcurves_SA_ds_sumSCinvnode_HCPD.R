...
## The data will be used to draw developmental trajectories. (Supplementary Figure 4)
library(R.matlab)
library(mgcv)
library(psych)
library(tidyverse)
...
ggplot()+
  ...
  theme(axis.text=element_text(size=15, color="black"), 
        axis.title =element_text(size=15, color="black"),aspect.ratio = 0.9,
        plot.background=element_rect(fill="transparent"),
        panel.background=element_rect(fill="transparent"),
        plot.title = element_text(size=15, hjust = 0.5), legend.position = "none")

ggsave(paste0(FigureFolder, '/CV',CVthr, '/SA', ds.resolution, '_sumSCinvnode_fit/devcurve_Rsq_fit.ratio.tiff'), width=20, height =14, units = "cm")
ggsave(paste0(FigureFolder, '/CV',CVthr, '/SA', ds.resolution, '_sumSCinvnode_fit/devcurve_Rsq_fit.ratio.svg'), dpi=600, width=15, height =15, units = "cm")
# colored based on mean 2nd derivative
colorbarvalues.meanderiv2 <- colorbarvalues(plotdatasum.df$meanderiv2, 0.38)
...
ggsave(paste0(FigureFolder, '/CV',CVthr, '/SA', ds.resolution, '_sumSCinvnode_fit/devcurve_SCrank_fit.Z.tiff'), width=20, height =14, units = "cm")
ggsave(paste0(FigureFolder, '/CV',CVthr, '/SA', ds.resolution, '_sumSCinvnode_fit/devcurve_SCrank_fit.Z.svg'), dpi=600, width=16, height =16, units = "cm")
...
ggsave(paste0(FigureFolder,'/CV',CVthr,  '/SA', ds.resolution, '_decile_sumSCinvnode_fit/devcurve_SCrank_fit.Z_SCtype10.tiff'), dpi=600, width=20, height =14, units = "cm")
ggsave(paste0(FigureFolder,'/CV',CVthr,  '/SA', ds.resolution, '_decile_sumSCinvnode_fit/devcurve_SCrank_fit.Z_SCtype10.svg'), dpi=600, width=15, height =13, units = "cm")
