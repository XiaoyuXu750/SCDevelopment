clc;
clear;
% this script is to downsample S-A cortex to 7 parcels
%% schaefer 400*7
...
schaefer400dlabel_7axis = schaefer400dlabel;
%SA rank
...
edges = linspace(1, 376, 8);
schaefer400_SA.SA7node = discretize(schaefer400_SA.SAaxisrank, edges);

schaefer400dlabel_cdata = schaefer400dlabel.cdata;
schaefer400dlabel7_cdata = zeros(length(schaefer400dlabel_cdata),1);
for i = 1:7
    regionidx = schaefer400_SA.index(find(schaefer400_SA.SA7node == i));
    
    % 计算区间的起始索引和结束索引
    Idx = find(ismember(schaefer400dlabel_cdata, regionidx));
    
    % 对区间内的元素赋予编码
    schaefer400dlabel7_cdata(Idx) = i;
end

tabulate(schaefer400dlabel7_cdata)
schaefer400dlabel_7axis.cdata=schaefer400dlabel7_cdata;

tablelabel=schaefer400dlabel.diminfo{1,2}.maps.table;
tablenew=struct;
tablenew(1).name='???'; tablenew(1).key=0; tablenew(1).rgba=tablelabel(1).rgba;
colors = brewermap(7, 'RdBu');
colors = colors([7:-1:1],:);
for i=1:7
...
schaefer400dlabel_7axis.diminfo{1,2}.maps.table=tablenew;

SAatlaspath='/Users/xuxiaoyu_work/Cuilab/Atlas/S_A_axis/S-A_ArchetypalAxis/FSLRVertex';
cifti_write(schaefer400dlabel_7axis, [SAatlaspath, '/SensorimotorAssociation_schaefer400_Axis7.dlabel.nii']);

%% schaefer400_SA7 fslr to fsaverage
...
ciftiinput=[fslrpath, 'SensorimotorAssociation_schaefer400_Axis7.dlabel.nii'];
ciftioutput_L=[fslrpath, 'SensorimotorAssociation_schaefer400_Axis7_LH.fslr32k.label.gii'];
ciftioutput_R=[fslrpath, 'SensorimotorAssociation_schaefer400_Axis7_RH.fslr32k.label.gii'];
...
metricout_L=[freesurferpath, 'SensorimotorAssociation_schaefer400_Axis7_LH.fsaverage5.label.gii'];
metricout_R=[freesurferpath, 'SensorimotorAssociation_schaefer400_Axis7_RH.fsaverage5.label.gii'];
...
%% run in shell
% cp /mnt/nas/xuxiaoyu/Atlas/SensorimotorAssociation_schaefer400_Axis7* /mnt/f/software/freesurfer/subjects/fsaverage5/label/
% cd /mnt/f/software/freesurfer/subjects/fsaverage5/label/
% mris_convert --annot SensorimotorAssociation_schaefer400_Axis7_LH.fsaverage5.label.gii ../surf/lh.sphere SensorimotorAssociation_schaefer400_Axis7_LH.fsaverage5.annot
% mris_convert --annot SensorimotorAssociation_schaefer400_Axis7_RH.fsaverage5.label.gii ../surf/rh.sphere SensorimotorAssociation_schaefer400_Axis7_RH.fsaverage5.annot
% 

%% extract centroid
addpath(genpath('/Users/xuxiaoyu_work/Cuilab/matlab_toolbox/rotate_parcellation/'));
path_annot_lh = [freesurferpath,'lh.SensorimotorAssociation_schaefer400_Axis7_LH.fsaverage5.annot'];
path_annot_rh = [freesurferpath,'rh.SensorimotorAssociation_schaefer400_Axis7_RH.fsaverage5.annot'];
path_sphere_lh = [freesurferpath,'lh.sphere'];
path_sphere_rh = [freesurferpath,'rh.sphere'];

sphere_SA7_lh = centroid_extraction_sphere(path_sphere_lh, path_annot_lh);
sphere_SA7_rh = centroid_extraction_sphere(path_sphere_rh, path_annot_rh);
sphere_SA7_lh(1, :) =[];
sphere_SA7_rh(1, :) =[];

resultFolder = '/Users/xuxiaoyu_work/Cuilab/GeneralRfunctions/rotate_parcellation/';
save('sphere_SA7_lh.mat', 'sphere_SA7_lh');
save('sphere_SA7_rh.mat', 'sphere_SA7_rh');
writematrix(sphere_SA7_lh, [resultFolder, 'sphere_SA7_lh.csv']);
writematrix(sphere_SA7_rh, [resultFolder, 'sphere_SA7_rh.csv']);
