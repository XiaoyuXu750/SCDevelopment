...
sum(gamresult$pfdr < 0.05) #71 edges sig
...
ggplot(data=correlation.df)+
  ...
  scale_color_distiller(type="seq", palette = "RdBu", direction = -1, limits=c(-max(correlation.df$meanderv2), max(correlation.df$meanderv2)))+
...
## 4. matrix graphs for resolution at 12
...
for (computevar in computevarlist){
  ...
  ggsave(filename, Fig,  height = 16, width = 18, units = "cm")
