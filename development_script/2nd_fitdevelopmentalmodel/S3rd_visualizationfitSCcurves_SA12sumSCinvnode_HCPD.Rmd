...
# rbind plotdata
gamresultsum.SAorder.delLM <- gamresultsum.SAorder.delLM %>% mutate(meanderv2_c = case_when(
...
for (i in 1:78){
  ...
  tmp$meanderiv2<-gamresultsum.SAorder.delLM$meanderv2_c[i]
...
ggplot()+
  ...
  scale_color_distiller(type="seq", palette = "RdBu",limits=c(-max(abs(plotdatasum.df$meanderiv2), na.rm = T), max(abs(plotdatasum.df$meanderiv2), na.rm = T)), na.value = "#2166AC", direction = -1)+
