...
# input which consistency threshold should be analyzed
CVthr=75
...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  # all TRUE 69, FALSE 9, the bootstrap.p value is not stable
  summary(gamresultsum.df)
  
  saveRDS(gamresultsum.df, paste0(interfileFolder, '/gamresults', elementnum, '_sumSCinvnode_over8_siteall_CV',CVthr, '.rds'))
}else{
  gamresultsum.df <- readRDS(paste0(interfileFolder, '/gamresults', elementnum, '_sumSCinvnode_over8_siteall_CV',CVthr, '.rds'))
}
...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  saveRDS(resultsum, paste0(interfileFolder, '/gammodel', elementnum, '_sumSCinvnode_over8_siteall_CV',CVthr,'.rds'))
}else{resultsum <- readRDS(paste0(interfileFolder, '/gammodel', elementnum, '_sumSCinvnode_over8_siteall_CV',CVthr,'.rds'))}
...
if (str_detect(wdpath, "cuizaixu_lab")){
  ...
  SCdata.diw[,c("gender", "handness", "race")] <- lapply(SCdata.diw[,c("gender", "handness", "race")], as.factor)
  covariates<-"gender+handness+race+mean_fd"
  dataname<-"SCdata.diw"
  smooth_var<-"age"
  resultsum <- list()
  ...
  class(gamresultsum.df$partialRsq)
  summary(gamresultsum.df)
  saveRDS(gamresultsum.df, paste0(interfileFolder, '/gamresults', elementnum, '_sumSCinvnode_over8_siteall_CV', CVthr,'_scale_TRUE.rds'))
}
