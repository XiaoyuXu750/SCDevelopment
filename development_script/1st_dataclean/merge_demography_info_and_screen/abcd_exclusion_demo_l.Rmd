...
```
### Age & Sex
Exclude participants with empty Age (NA), and atypical Sex (3, 4, 999).
...
idx_age <- is.na(demo_data$demo_brthdat_v2)

demo_data$eventname2 <- gsub("baseline_year_1_arm_1", "baselineYear1Arm1", demo_data$eventname)
demo_data$eventname2 <- gsub("2_year_follow_up_y_arm_1", "2YearFollowUpYArm1", demo_data$eventname2)
demo_data$scanID <- paste0(gsub("NDAR_", "sub-NDAR", demo_data$src_subject_id),"_ses-", demo_data$eventname2)

sub_age <- demo_data$scanID[idx_age]
print(paste0('Invalid age: exclude ', sum(idx_age), ' participants' ))

```
```{r}
demo_data <- within(demo_data, 
                    {demo_sex_v2[eventname=='2_year_follow_up_y_arm_1'] <-demo_gender_id_v2_l[eventname=='2_year_follow_up_y_arm_1']})

idx_sex <- !with(demo_data, demo_sex_v2==1 | demo_sex_v2==2)
idx_sex[is.na(idx_sex)] <- TRUE
sub_sex <- demo_data$scanID[idx_sex]
print(paste0('Atypical sex: exclude ', sum(idx_sex), ' participants' ))

```
```{r}
racevar <- names(demo_data)[str_detect(names(demo_data), 'demo_race')]
demo_data_b <- demo_data[demo_data$eventname=="baseline_year_1_arm_1",]
demo_sublist <- demo_data[,c("src_subject_id", "eventname", "scanID")]
demo_sublist$race <- NA
for (i in 1:nrow(demo_sublist)){
  subID <- demo_sublist$src_subject_id[i]
  index <- which(demo_data_b$src_subject_id==subID)
  racevactor <- as.numeric(demo_data_b[index, racevar])
  raceindx <- which(racevactor > 0)
  if (length(raceindx)>1){
    raceindx <- raceindx[1]
  }
  if (length(raceindx)==1){
    if (raceindx==1){
      demo_sublist$race[i] <- "white"
    }else if (raceindx==2){
      demo_sublist$race[i] <- "black"
    }else if (raceindx %in% c(9:15)){
      demo_sublist$race[i] <- "asian"
    }else if (raceindx %in% c(17,18)){
      demo_sublist$race[i] <- NA
    }else{
      demo_sublist$race[i] <- "other"
    }
  }else{demo_sublist$race[i] <- NA}
  
}

idx_race <- is.na(demo_sublist$race)
num_race <- sum(idx_race)

print(paste0('Invalid race: exclude ', num_race, ' participants' ))
```
...
demo_sublist$if_age <- demo_sublist$scanID %in% sub_age
