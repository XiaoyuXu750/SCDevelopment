...
detectCores()
# load data
SCdata<-readRDS(paste0(interfileFolder, '/SCdata_SA12_CV75_sumSCinvnode.sum.msmtcsd.combatgam_pfactor.rds'))
...
if (str_detect(wdpath, "cuizaixu_lab")){
  resultsum <- mclapply(1:78, function(x){
    region <- grep("SC.", names(SCdata), value=T)[x]
    gamresult <- gamm.smooth.predict.covariateinteraction(region, dataname, smooth_var, int_var, int_var.predict.percentile, covariates, knots, set_fx = T, increments)
...
for (Interest.var in c("IntpartialRsq", "T.disease")){
  ...
  ggsave(filename, MatFig,  height = 15, width = 16, units = "cm")
