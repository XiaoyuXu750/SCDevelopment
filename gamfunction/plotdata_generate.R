plotdata_generate <- function(modobj,smooth_var){
  ...
  pred$fit.C <- scale(pred$fit, center = T, scale = F)
  pred$fit.Z <- scale(pred$fit, center = T, scale = T)
  pred$fit.floor <- pred$fit-pred$fit[1]
  pred$fit.ratio <- pred$fit / pred$fit[1]
