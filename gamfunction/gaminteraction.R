...
gam.smooth.predict.covariateinteraction <- function(region, dataname, smooth_var, int_var, int_var.predict.percentile, covariates, knots, set_fx = FALSE, increments, stats_only=TRUE){
  ...
  predicted.smooth <- predicted.smooth %>% dplyr::select(all_of(smooth_var), fitted, se, lower, upper, fitted.centered)
  ...
  # #age effect size
  #####################
  # divide gam.data to 3 equal parts according to the cognitive levels
  int<-gam.data[ ,int_var]
  cuttoff_3 <- quantile(int, probs=c(0,0.33333333, 0.6666666666, 1))
  gam.data.sep<-list()
  anova.smooth.sep.pvalue<-rep(0,3)
  partialRsq.sep<-rep(0,3)
  for (i in 1:3){
    gam.data.sep[[i]]<-gam.data[which(int>=cuttoff_3[i] & int<cuttoff_3[i+1]), ]
  }
  for (i in 1:3){
    modelformula.sep <- as.formula(sprintf("%1$s ~ s(%2$s, k=%3$s, fx=%4$s)+ %5$s", region, smooth_var, knots, set_fx, covariates))
    modelformula.sep.null <- as.formula(sprintf("%1$s ~ %2$s", region, covariates))
    gam.model.sep <- gam(modelformula.sep, method = "REML", data = gam.data.sep[[i]])
    gam.model.sep.null <- gam(modelformula.sep.null, method = "REML", data = gam.data.sep[[i]])
    ##Full versus reduced model anova p-value
    anova.smooth.sep.pvalue[i] <- anova.gam(gam.model.sep,gam.model.sep.null,test='Chisq')$`Pr(>Chi)`[2]
    
    ##Full versus reduced model direction-dependent partial R squared
    ### effect size
    sse.model <- sum((gam.model.sep$y - gam.model.sep$fitted.values)^2)
    sse.nullmodel <- sum((gam.model.sep.null$y - gam.model.sep.null$fitted.values)^2)
    partialRsq.sep[i] <- (sse.nullmodel - sse.model)/sse.nullmodel
    
  }
  anova.smooth.sep.pvalue.level1 <-anova.smooth.sep.pvalue[1]
  anova.smooth.sep.pvalue.level2 <-anova.smooth.sep.pvalue[2]
  anova.smooth.sep.pvalue.level3 <-anova.smooth.sep.pvalue[3]
  partialRsq.sep.level1 <- partialRsq.sep[1]
  partialRsq.sep.level2 <- partialRsq.sep[2]
  partialRsq.sep.level3 <- partialRsq.sep[3]
  ...
  stats.reults<-cbind(parcel, int_var, anova.int.pvalue, gam.int.pvalue, IntpartialRsq,Int.F, changed.range, 
                      ...
                      peak.change, anova.smooth.sep.pvalue.level1, anova.smooth.sep.pvalue.level2, anova.smooth.sep.pvalue.level3,
                      partialRsq.sep.level1, partialRsq.sep.level2, partialRsq.sep.level3)
