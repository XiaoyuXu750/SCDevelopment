...
gam.fit.smooth <- function(region, dataname, smooth_var, covariates, knots, set_fx = FALSE, stats_only = FALSE, mod_only = FALSE){
  ...
  #region <- sprintf("%s.%s", 'SC', as.character(region))
  ...
  derv <- derivatives(gam.model, term = sprintf('s(%s)',smooth_var), interval = "simultaneous", unconditional = F, data = pred) #derivative at 200 indices of smooth_var with a simultaneous CI
  derv_EPS <- derivatives(gam.model, term = sprintf('s(%s)',smooth_var), interval = "simultaneous", unconditional = F, data = pred2)
  derv2 <- (derv_EPS$derivative - derv$derivative) / EPS
  meanderv2<-mean(derv2)
